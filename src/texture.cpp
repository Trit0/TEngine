...
namespace te {
    ...
    Texture::Texture(Device &device, std::array<std::string, 6> textureFilePaths) : mDevice{device} {
        createTextureImageCubeMap(textureFilePaths);
        createTextureImageViewCubeMap(VK_IMAGE_VIEW_TYPE_2D);
        createTextureSampler();
        updateDescriptor();
    }
    ...
    std::unique_ptr<Texture> Texture::createTextureFromFiles(Device &device, std::array<std::string, 6> textureFilePaths) {
        return std::make_unique<Texture>(device, textureFilePaths);
    }
    ...
    void Texture::createTextureImageCubeMap(std::array<std::string, 6> &textureFilePaths) {
        int texWidth, texHeight, texChannels;
        mLayerCount = 6;
        stbi_uc *textureData[mLayerCount];
        VkDeviceSize layerSizes[mLayerCount];
        VkDeviceSize imageSize = 0;

        for (int i = 0; i < mLayerCount; i++) {
            textureData[i] = stbi_load(textureFilePaths[i].c_str(), &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
            layerSizes[i] = texWidth * texHeight * 4;
            imageSize += layerSizes[i];
        }
        ...
        for (int i = 0; i < mLayerCount; i++) {
            memcpy(static_cast<char*>(data) + (offset), textureData[i], static_cast<size_t>(layerSizes[i]));
            offset += layerSizes[i];
        }
    ...
    void Texture::createTextureImageViewCubeMap(VkImageViewType viewType) {
        VkImageViewCreateInfo viewInfo{};
        viewInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
        viewInfo.image = mTextureImage;
